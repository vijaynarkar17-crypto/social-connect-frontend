...
export default function FollowersPage() {
  ...
  useEffect(() => {
    if (username) api.get(`/api/users/${username}/followers`).then(({ data }) => setUsers(data.users));
  }, [username]);
    ...
    <AppShell>
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="glass-card divide-y divide-gray-100 dark:divide-gray-800 !p-0">
          ...
          {users.length === 0 && <p className="p-8 text-center text-gray-500">No followers yet</p>}
        </div>
      </div>
    </AppShell>
  );
